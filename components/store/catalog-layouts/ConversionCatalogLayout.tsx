...
/**
 * Tema "Conversión general" — layout de catálogo base (el mismo que ya
 * existía antes de separar temas estructurales): filtros en dropdown +
 * drawer móvil, grilla estándar de 2 a 4 columnas.
 */
export function ConversionCatalogLayout({
  category,
  setCategory,
  sort,
  setSort,
      ...
      <main className="mx-auto max-w-7xl px-3 py-10 sm:px-6 lg:px-8">
        ...
        {displayed.length === 0 ? (
          <div className="flex flex-col items-center justify-center gap-4 py-24 text-center">
            ...
            <div>
              <p className="font-semibold text-[var(--color-text)]">No hay productos disponibles</p>
              <p className="mt-1 text-sm text-[var(--color-text-muted)]">Intenta con otra categoría u orden.</p>
            </div>
            <Link href="/">
          ...
          <>
            ...
            >
              ...
              <div className="mb-6 flex items-center justify-between">
                <h2 className="font-display text-lg font-bold text-[var(--color-text)]">Filtros</h2>
                <button
                  onClick={() => setDrawerOpen(false)}
              ...
              <div className="mb-8">
                ...
                <div className="flex flex-col gap-2">
                  {SORT_OPTIONS.map(({ value, label }) => (
                    ...
                    >
                      {label}
                      {sort === value && <span className="h-2 w-2 rounded-full bg-[var(--color-primary)]" />}
                    </button>
                  ))}
                </div>
              </div>
              ...
              <Button fullWidth onClick={() => setDrawerOpen(false)}>
                Ver {displayed.length} {displayed.length === 1 ? "producto" : "productos"}
              </Button>
            </motion.div>
          </>
        )}
